...
def extract_cookie_jar_from_browser(
    cookies_from_browser: str,
) -> requests.cookies.RequestsCookieJar:
    browser_name, profile, keyring, container = parse_cookies_from_browser_arg(
        cookies_from_browser
    )
    yt_dlp_cookie_jar: yt_dlp.cookies.YoutubeDLCookieJar = (
        yt_dlp.cookies.extract_cookies_from_browser(
            browser_name=browser_name,
            profile=profile,
            logger=PatchedYDLLogger(),
            keyring=keyring,
            container=container,
        )
    )
...
def parse_cookies_from_browser_arg(
    cookies_from_browser: str,
    ...
    if mobj is None:
        raise ValueError(
            f"invalid cookies from browser arguments: {cookies_from_browser}"
        )
