...
HOLOPLUS_GOOGLE_KEY = os.getenv(
    "HOLOPLUS_GOOGLE_KEY", default="AIzaSyBIBy1CboBwrCShfY1CixfRRynJRF06vx0"
)
...
def auth_token(
    cookie_jar: requests.cookies.RequestsCookieJar, timeout: int = 30
) -> AuthTokenResponse:
    """
    ...
    with requests.Session() as session:
        ...
        response = session.request(
            method="GET", url=account_auth_url, timeout=timeout, allow_redirects=False
        )
        api_callback_url = response.headers.get("Location")

        if response.status_code != 302:
            raise HoloplusAuthError(
                "Unexpected auth status code", account_auth_url, response.status_code
            )
        ...
        if not api_callback_url or not api_callback_url.startswith(
            "https://api.holoplus.com/v2/auth/callback?"
        ):
            raise HoloplusAuthError(
                "Unexpected auth redirect", account_auth_url, api_callback_url
            )
        ...
        response = session.request(
            method="GET", url=api_callback_url, timeout=timeout, allow_redirects=False
        )
        holoplus_url = response.headers.get("Location")

        if response.status_code != 302:
            raise HoloplusAuthError(
                "Unexpected auth status code", api_callback_url, response.status_code
            )

        if not holoplus_url or not holoplus_url.startswith("holoplus://"):
            raise HoloplusAuthError(
                "Unexpected auth redirect", api_callback_url, holoplus_url
            )
